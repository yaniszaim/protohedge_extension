...
class Monitor(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end( self, loop_epoch, logs = None ):
        ...
        if self.progress_data.epoch % 2 == 0:  # print every 10 epochs
            if hasattr(self.gym.agent, "print_sample_prototype_actions"):
                self.gym.agent.print_sample_prototype_actions(indices=[0, 50, 218])
